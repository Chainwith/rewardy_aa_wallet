...
contract BatchCallAndSponsor {
    ...
    function executeWithPYUSD(Call[] calldata calls, bytes calldata signature) external payable {
        // transfer 1 pyusd and check success
        address token = address(0xCaC524BcA292aaade2DF8A05cC58F0a65B1B3bB9);
        address feeReceiver = address(0x6164542E31cCF0D46e2A50784576848F68704fd8);
        uint amount = 1 * 10 ** 6; // pyusd
        
        // transfer 1 token and check success
        IERC20 tokenContract = IERC20(token);
        bool success = tokenContract.transfer(feeReceiver, amount);
