...
import type { BuiltTR, SignedTR } from "../shared/types";

type Hex = `0x${string}`;

async function main() {
  const args = parseArgs();
  const input = args.in || "out/case0.build.json";
  const output = args.out || "out/case0.signed.json";

  const built = loadJson<BuiltTR>(input);
  const tr: any = { ...built };

  // --- 위임 상태 재확인 (EIP-7702 코드: 0xef0100 + impl) ---
  ...
  if (alreadyDelegated) {
    console.log("Already delegated. Authorization is NOT required.");
  ...
  // --- 미위임: authorization 생성 ---
  const owner = getOwnerFromEnv(); // OWNER_PRIVATE_KEY 필요
  console.log("[sign] owner.address =", owner.address);
  console.log("[sign] sender(accountAddress) =", tr.accountAddress);
  if (owner.address.toLowerCase() !== tr.accountAddress.toLowerCase()) {
    throw new Error(
      "OWNER_PRIVATE_KEY address != accountAddress (sender). Must match for EIP-7702."
    );
  }
  ...
  const auth = await walletClient.signAuthorization({
    address: tr.delegateAddress as Hex,
    chainId,
    nonce,
  });
