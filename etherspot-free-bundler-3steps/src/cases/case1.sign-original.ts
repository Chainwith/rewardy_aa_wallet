// src/cases/case1.sign.ts  (case2/3/4는 파일명과 기본 in/out만 변경)
...
async function main() {
  ...
  const input = args.in || "out/case1.build.json";
  const output = args.out || "out/case1.signed.json";
  ...
  const senderCode = await publicClient.getCode({
    address: tr.accountAddress as Hex,
  });
  const expectedPrefix = `0xef0100${tr.delegateAddress
    .toLowerCase()
    .slice(2)}` as Hex;
  ...
  //   saveJson(output, tr);
  //   console.log(`📝 saved: ${output} (already delegated)`);
  //   return;
  // }
