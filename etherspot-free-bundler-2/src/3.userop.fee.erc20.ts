// case3.sponsor.erc20fee.erc20.ts
// 4337 + Paymaster 스폰서 + ERC20 수수료 + ERC20 전송
...
async function main() {
  const SELECTED_FEE_TOKEN = envOrThrow('SELECTED_FEE_TOKEN') as `0x${string}`; // 수수료 토큰(등록된)
  const TRANSFER_TOKEN     = envOrThrow('TRANSFER_TOKEN') as `0x${string}`;     // 전송할 ERC20
  const TRANSFER_TO        = envOrThrow('TRANSFER_TO') as `0x${string}`;
  const TRANSFER_AMOUNT    = process.env.TRANSFER_AMOUNT ?? '1';                // 사람 단위
  ...
  // Calls = [ selectFeeToken(self-call) , ERC20 transfer ]
  const selectFeeCall = {
    to: smartAccount.address,
    value: 0n,
    data: encodeFunctionData({ abi: accountAbi, functionName: 'selectFeeToken', args: [SELECTED_FEE_TOKEN] }) as Hex,
  };
  ...
  const calls = [selectFeeCall, erc20Transfer];
